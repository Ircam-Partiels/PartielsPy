...
- Analyze audio files for spectral content
...
Partiels and its plugin suite can be downloaded from the **[Github project page of Partiels](https://github.com/Ircam-Partiels/Partiels)**
## Installation
...
For more control over the logging output:
...
# Only show PartielsPy-related logs
logger = logging.getLogger('partielspy')
...
PartielsPy loggin only uses the DEBUG level. To disable the debug messages you can set the logging level to any other level:
- `INFO`
- `WARNING`
- `ERROR`
- `CRITICAL`
...
PartielsPy provides a simple, flexible API for analyzing audio files and exporting the results in various formats. Here are some examples showing how to use the library:
...
# Initialize the Partiels analyzer
partiels = Partiels()
...
Export spectral analysis as visual representations:
...
```python
# Analyze once, export in multiple formats
...
You can process multiple audio files with a single export configurations:

```python
# export mutiple audio files with a single export format
config = ExportConfigImage()
partiels.export(audio_file_1, template_file, output_folder, config)
partiels.export(audio_file_2, template_file, output_folder, config)
partiels.export(audio_file_3, template_file, output_folder, config)
```


TODO

- Relire & éditer (ne pas tout garder)
- Example d'affichage d'une image
- Example d'affichage d'un tableau CSV